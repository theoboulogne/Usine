...
class Joueur{
    ...
    barres(nbTour){
        ...
        if(this.Choix.norme.pollution != -1){
            if(this.Choix.norme.pollution > this.Eco.pollution) Ecologie *= 0.5
        }
        if(this.Choix.norme.dechets != -1){
            if(this.Choix.norme.dechets > this.Eco.dechets) Ecologie *= 0.5
        }
        ...
        for(let i=0; i<this.Lignes.length; i++){
            for(let j=0; j<5; j++){
                if(this.Lignes[i].Composant[j].auto) auto++
            }
        }
